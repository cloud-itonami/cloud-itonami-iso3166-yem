...
(deftest register-draft-and-submit
  (let [d (registry/register-draft "eng-1" "YEM" 0)
        s (registry/register-submit "eng-1" "YEM" 0)]
    (is (= "YEM-DFT-000000" (get d "draft_number")))
    (is (= "YEM-SUB-000000" (get s "submit_number")))
    (is (nil? (get-in d ["certificate" "proof"])))
    (is (= "draft-unsigned" (get-in s ["certificate" "status"])))))

(deftest register-requires-ids
  (is (thrown? Exception (registry/register-draft "" "YEM" 0)))
