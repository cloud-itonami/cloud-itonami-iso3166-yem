  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Sabaean Trading Co"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Sabaean Trading Co" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest non-yemeni-workforce-quota-exceeded-is-held-and-unoverridable
  (testing "non-Yemeni workforce 3/10 = 30% exceeds Labor Law Art.21's 10% cap -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:non-yemeni-workforce-quota-exceeded} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest non-yemeni-workforce-quota-check-not-triggered-for-domestic-only-engagement
  (testing "an engagement that never declares foreign-hiring intent is exempt from the flagship check"
    (let [[db actor] (fresh)
          _ (assess! actor "t6pre" "eng-6")
          _ (draft! actor "t6pre" "eng-6")
          res (exec-op actor "t6" {:op :filing/submit :subject "eng-6"} operator)]
      (is (= :interrupted (:status res)) "clean engagement still always escalates for actuation, but not held")
      (let [r2 (approve! actor "t6")]
        (is (= :commit (get-in r2 [:state :disposition])))
        (is (not (some #{:non-yemeni-workforce-quota-exceeded} (mapcat :basis (store/ledger db))))
            "the flagship check never fires for an engagement that doesn't declare foreign-hiring intent")))))
...
(deftest cr-unverified-is-held-and-unoverridable
  (testing "unverified Commercial Registry number when required -> HARD hold"
    (let [[db actor] (fresh)
          _ (assess! actor "t8pre" "eng-5")
          _ (draft! actor "t8pre" "eng-5")
          res (exec-op actor "t8" {:op :filing/submit :subject "eng-5"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
...
(deftest submit-always-escalates-then-human-decides
  (testing "a clean fully-assessed submit still ALWAYS interrupts for human approval"
    (let [[db actor] (fresh)
          ...
          _ (draft! actor "t9pre" "eng-1")
          r1 (exec-op actor "t9" {:op :filing/submit :subject "eng-1"} operator)]
...
(deftest draft-always-escalates-then-human-decides
  (testing "a clean fully-assessed draft still ALWAYS interrupts for human approval"
    (let [[db actor] (fresh)
          ...
          r1 (exec-op actor "t10" {:op :filing/draft :subject "eng-1"} operator)]
      (is (= :interrupted (:status r1)) "pauses for human approval even when governor-clean")
...
(deftest engagement-double-draft-is-held
  (testing "drafting the same engagement twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          ...
          res (exec-op actor "t11" {:op :filing/draft :subject "eng-1"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
...
(deftest engagement-double-submit-is-held
  (testing "submitting the same engagement twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          ...
          _ (exec-op actor "t12a" {:op :filing/submit :subject "eng-1"} operator)
          _ (approve! actor "t12a")
          res (exec-op actor "t12" {:op :filing/submit :subject "eng-1"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
